		...
		if($validate){
			...
			if($update){
				...
				if($userName_change == TRUE){
					...
					$eski_siparisler = $this->siparisler_model->get_all(
						array(
							"eczaci" => $oldUser->user_name
						),
					);
					
					foreach($eski_siparisler as $eski_siparis){
						
						$update = $this->siparisler_model->update(
							array("id" => $eski_siparis->id),
							array(
								"eczaci" => $this->input->post("user_name"),
								)
						);
					}
				}
